...
class Xkdc extends Social {
  ...
  async run(message, args, level) { // eslint-disable-line no-unused-vars
    ...
    if (message.flags[0] == "f") {
      ...
      return message.channel.send({"embed": {"author":{"name":"REmibot | XKCD Comics"}, "description":`${ab.title} | #${ab.num}\n${ab.alt}`, "image":{"url":ab.img}}});
    }
    ...
    message.channel.send({"embed": {"author":{"name":"REmibot | Daily XKCD Comics"}, "description":`${ob.title} | #${ob.num}\n${ob.alt}`, "image":{"url":ob.img}}});
  }
}
