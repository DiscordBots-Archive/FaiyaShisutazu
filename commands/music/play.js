...
module.exports = class Play extends Command {
  ...
  async run(message, args) { // eslint-disable-line no-unused-vars
    try {
      ...
      const song = args;
      if (!song.length) return message.channel.send(oneLine`
        <:tsukihi:559908175906734097> B-baka! Input something **${message.member.displayName}-san**!
      `);
      if (song.search(/(www\.)?youtube\.com\/playlist\?list=/) !== -1) return message.channel.send(oneLine`
      ...
      if (id) await addSong();
        ...
        collector.on("end", async (collected) => {
          ...
          if (collected.first()) {
            if (collected.first().emoji.name === "❎") await message.channel
